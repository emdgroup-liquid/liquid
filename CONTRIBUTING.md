...
- When ever possible, try to provide CSS components alongside Web components using the same CSS file; again, prefix the classes with `ld-`, use BEM and do **not** use shadow dom. Here is an example:   ```tsx
...
- We enable type checking and intelliSense for Web Component attributes by importing the autogenerated components type definitions file (src/components.d.ts) at the top of all imports in each component:   ```tsx
...
> > If you can not or do not want to use Docker, you can still work around the > _"Component tag name must be unique"_ issue by deleting the dist folder. > Note though, that it may eventually be recreated, for instance if you run tests.
