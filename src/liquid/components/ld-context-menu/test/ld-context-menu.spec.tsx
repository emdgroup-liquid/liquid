...
const prepareAndGetMenuInTooltip = async (
  ...
  const triggerSlot =
    page.root.shadowRoot.querySelector<AnyHTMLElement>("slot");
  const defaultSlot =
  ...
  tooltipTriggerSlot.assignedElements = jest
    .fn()
    .mockReturnValue([triggerSlot]);
...
describe("ld-context-menu", () => {
  ...
  it("closes on right-click of other context menu", async () => {
    ...
    page.body.dispatchEvent(
      new CustomEvent("ldtooltipopen", { bubbles: true }),
    );
    await page.waitForChanges();
